...
class App(tk.Tk):
    def __init__(self):
        ...
        self.draw_polygons_method = None
    ...
    def mouse_pressed(self, event) -> None:
        ...
        if not self.is_multiple_dots:
            if self.begin is None:
                ...
                for dot in dots:
                    if self.debug.get():
                        self.canvas.create_rectangle(dot.x * 10, dot.y * 10, dot.x * 10 + 10, dot.y * 10 + 10,
                                                     fill=color_intesity[dot.i], tags='dline')
                        print(dot.x, dot.y, dot.i)
                        self.update()
                        time.sleep(0.5)
                    else:
                        self.canvas.create_rectangle(dot.x, dot.y, dot.x + 1, dot.y + 1, fill=color_intesity[dot.i],
                                                     tags='line')
                if not self.debug.get():
                    self.lines.append(dots)
                self.begin = None
        elif self.is_multiple_dots and self.draw_polygons_method is None:
            if self.debug.get():
            ...
            if len(self.dots) == self.dots_need.get():
                ...
                for i in range(1, len(dots)):
                    if self.debug.get():
                        self.canvas.create_rectangle(dots[i - 1].x * 10, dots[i - 1].y * 10, dots[i].x * 10,
                                                     dots[i].y * 10, fill=color_intesity[dots[i - 1].i], tags='dcurve')
                        print(dots[i].x, dots[i].y, dots[i].i)
                        self.update()
                        time.sleep(0.5)
                    else:
                        self.canvas.create_line((dots[i - 1].x, dots[i - 1].y), (dots[i].x, dots[i].y),
                                                fill=color_intesity[dots[i].i], tags='curve')
                if not self.debug.get():
                    cur = curve.Curve()
                    cur.dots = dots
                    i = 0
                    for dot in self.dots:
                        cur.boundary_points['P' + str(i)] = [
                            self.canvas.find_overlapping(dot.x - 4, dot.y - 4, dot.x + 4, dot.y + 4)[0],
                            dot
                        ]
                        i += 1
                    self.curves.append(cur)
                self.dots.clear()
        elif self.is_multiple_dots and self.draw_polygons_method is not None:
            self.dots.append(line.Dot(x=x, y=y))
            if len(self.dots) == self.dots_need.get():
                pol = Polygon(self.dots_need.get())
                pol.dots = copy(self.dots)
                self.draw_polygon(pol)
                if not self.debug.get():
        ...
        if self.begin is not None and not self.debug.get() and not self.is_multiple_dots:
            ...
            for obj in self.obj:
                for cur in self.curves:
                    for bp in cur.boundary_points:
                        if obj == cur.boundary_points[bp][0]:
                            cur.boundary_points[bp][1] = line.Dot(event.x, event.y)
    ...
    def draw_curves(self):
        for cur in self.curves:
            dots = self.draw_method(cur.get_boundary_dots())
            for i in range(1, len(dots)):
                self.canvas.create_line((dots[i - 1].x, dots[i - 1].y), (dots[i].x, dots[i].y),
                                        fill=color_intesity[dots[i].i], tags='curve')

    def draw_line(self, l: line.Line):
        dots = line.bresenham(l.begin, l.end)
        for dot in dots:
            self.canvas.create_rectangle(dot.x, dot.y, dot.x + 1, dot.y + 1, fill=color_intesity[dot.i],
                                         tags='polygon')
    ...
    def draw_boundary_pont(self, dot: line.Dot, i: int):
        if self.debug.get():
            self.canvas.create_oval(dot.x * 10 - 5, dot.y * 10 - 5, dot.x * 10 + 5, dot.y * 10 + 5, outline='black',
                                    tags='P' + str(i))
    ...
    def draw_boundary_points(self):
        self.clear_boundary_points()
        for i, dot in enumerate(self.dots):
            self.draw_boundary_pont(dot, i)
        for i, cur in enumerate(self.curves):
            dots = cur.get_boundary_dots()
            for dot in dots:
                self.draw_boundary_pont(dot, i)
        for pol in self.polygons:
            for i, dot in enumerate(pol.dots):
                self.draw_boundary_pont(dot, i)
    ...
    def draw_polygon(self, polygon: Polygon):
        if self.debug.get():
            ...
            for i, dot in enumerate(polygon.dots):
                self.draw_boundary_pont(dot, i)
    ...
    def draw_grid(self):
        if self.debug.get():
            for lin in range(0, self.width + 1, 10):
                self.canvas.create_line([(lin, 0), (lin, self.height)], fill='#828282', tags='grid_line_w')

            for lin in range(0, self.height + 1, 10):
                self.canvas.create_line([(0, lin), (self.width, lin)], fill='#828282', tags='grid_line_h')
        else:
            self.canvas.delete('grid_line_h', 'grid_line_w', 'dline', 'dcurve')

    def clear(self):
        self.canvas.delete('grid_line_h', 'grid_line_w', 'line', 'dline', 'curve', 'dcurve', 'polygon')
        self.lines.clear()
        self.curves.clear()
        self.polygons.clear()
        self.clear_boundary_points()
